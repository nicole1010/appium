...
在页面中注入一段JavaScript，以便在当前选定框架的上下文中执行 (Web context). 运行原生 [mobile command](/docs/en/commands/mobile-command.md) (Native Context).
[//]: # (DO NOT EDIT THIS FILE! This is an auto-generated file. Editing for this document happens in /commands-yml/commands/web/execute.yml)
## 使用示例
...
参数可以是任何JSON原语, 数组, 或者 JSON 对象。 定义WebElement引用的JSON对象将转换为相应的DOM元素。 同样地, 脚本结果中的任何WebElements都将作为WebElement JSON对象返回给客户端。

原生上下文（Native Context）中 `execute` , 详见 [Mobile Commands](/docs/en/commands/mobile-command.md).
...
|名称|描述|
|----|-----------|
|session_id|路由到该command的session的ID|
