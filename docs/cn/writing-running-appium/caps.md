...
|键|描述|值|
|----|-----------|-------|
|`espressoServerLaunchTimeout`|等待Espresso服务器启动的超时时间（以毫秒为单位）。默认值为 `30000` |例如 `50000`|
|`espressoBuildConfig`|Espresso服务器构建配置JSON的路径。(见下文)|例如 `/projects/myapp-tests/buildconfig.json`|
|`showGradleLog`|是否将Espresso服务器的Gradle构建日志通过管道传输到Appium日志。默认值为 `false`|例如 `true`|
|`skipServerInstallation`|跳过Espresso服务器构建和APK安装。此选项可能会破坏特定Appium版本的正确Espresso服务器设置，但是当设备上已经安装了正确的Espresso服务器和正确的被测试应用程序时，它可以提高启动性能。 如果Espresso服务器或被测应用程序需要更新，请确保不要启用此选项。 默认值为 `false` | `true` 或 `false`|
|`intentOptions`|Intent options 将用于启动被测应用程序。 例如JSON格式的 `action`, `categories` and `component`。 参见例子[#538](https://github.com/appium/appium-espresso-driver/issues/538)。 |例如 `{"action": "android.intent.action.MAIN", "categories": "android.intent.category.LAUNCHER", "component": "com.appium/.launcher.MainActivity"` |
|`disableSuppressAccessibilityService`| 设置[FLAG_DONT_SUPPRESS_ACCESSIBILITY_SERVICES](https://developer.android.com/reference/android/app/UiAutomation#FLAG_DONT_SUPPRESS_ACCESSIBILITY_SERVICES) 允许现有的可访问性服务继续运行，并为Appium启动一个新服务。它有助于测试具有可访问性功能（例如话语提示）的被测应用。 如果未提供任何内容，则Appium将不会指定该标志。该标志要求使用Android API Level 24+。 | `true`, `false` |
|`appLocale`| 自 Appium 1.18.0，设置 [地点](https://developer.android.com/reference/java/util/Locale) 为[目的上下文](https://developer.android.com/reference/androidx/test/core/app/ApplicationProvider#getApplicationContext()) 。 `language`, `country` 和 `variant` 和locale类用法相同。 `language` 是强制的。 详见 [locale](https://developer.android.com/reference/java/util/Locale)。设置`appLocale`仅会影响被测应用程序的语言环境，而不会影响其他系统视图，例如状态栏。 如果您想更改整个系统的语言环境，请设置 `language`, `locale` 且/或 `localeScript` 参数  | 例如 `{"language": "ja", "country": "JP"}` |
...
|`autoAcceptAlerts`| 当警告弹出的时候，都会自动去点接受。包括隐私访问权限的警告（例如 定位，联系人，照片）。默认值为 false。|`true`或`false`|
|`autoDismissAlerts`|当警告弹出的时候，都会自动去点取消。包括隐私访问权限的警告（例如 定位，联系人，照片）。默认值为 false。|`true`或`false`|
|`nativeInstrumentsLib`| 使用原生 intruments 库（即关闭 instruments-without-delay）。|`true`或`false`|
|`nativeWebTap`| 在Safari中允许"真实的"，非基于 javascript 的 web 点击 (tap) 。 默认值：`false`。注意：取决于 viewport 大小/比例， 点击操作不一定能精确地点中对应的元素。|`true`或`false`|
|`safariInitialUrl`|初始化 safari 的时使用的地址。默认是一个本地的欢迎页面 | 例如  `https://www.github.com` |
...
|`customSSLCert`|(仅支持模拟器) 给模拟器添加一个 SSL 证书。 | 例如  <br/>`-----BEGIN CERTIFICATE-----MIIFWjCCBEKg...`<br/>`-----END CERTIFICATE-----`|
|`webkitResponseTimeout`| (仅支持真机) 设置等待Safari会话中WebKit响应的时间（以毫秒为单位）。 默认值为 `5000`|例如 `10000`|
|`remoteDebugProxy`| (仅支持模拟器，且 <= 11.2) 如果已设置，则Appium 通过本地端口的代理(仅支持模拟器,且 <= 11.2)或者unix套接字上的代理(仅支持模拟器，且 >= 11.3)发送和接收远程调试消息。而不是直接与iOS远程调试器通信。|例如 `12000` 或 `"/tmp/my.proxy.socket"`|
